...
import { SiMysql, SiCplusplus, SiC, SiTailwindcss, SiVite, SiNextdotjs, SiVercel, SiFirebase } from "react-icons/si";
...
export function ToolsStack() {
    ...
    const leftColumn = tools.slice(0, midPoint);
    const rightColumn = tools.slice(midPoint);
    ...
    const renderItem = (item: ToolItem) => (
        ...
        >
            <div className="flex-shrink-0 p-2 bg-white/5 rounded-lg group-hover:bg-white/10 transition-colors">
                {item.icon}
            </div>
            <div className="flex flex-col text-left">
                <span className="font-bold text-lg text-white group-hover:text-purple-400 transition-colors">{item.name}</span>
        ...
        <section className="py-20 relative z-10">
            <div className="container mx-auto px-4">
                ...
                <div className="hidden md:grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
                    ...
                    <div>
                        <AnimatedList
                            items={rightColumn.map(renderItem)}
                            displayScrollbar={false}
                            showGradients={false}
                            className="w-full full-height"
                        />
